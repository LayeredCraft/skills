...
Keep dependencies directed inward: CLI, API, and I/O adapters may depend on core code; core code does not import them. Break cycles by moving shared contracts or data downward, or injecting a protocol. Parse environment, files, and arguments once at the boundary into typed configuration, then pass dependencies explicitly. Use `Path` for filesystem inputs, `importlib.resources` for package data, UTC-aware datetimes for timestamps, and monotonic clocks for durations and deadlines.
